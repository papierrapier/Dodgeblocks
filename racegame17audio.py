...
crash_sound = pygame.mixer.Sound('crash.wav') #loads any file for any sound use, perhaps pygame prefers wav, unsure.
pygame.mixer.music.load('racefast.mp3') # loads a file as use for music
...
carIcon = pygame.image.load('caricon.png') #directory for topleft icon display, also taskbar icon display

pygame.display.set_icon(carIcon) #game icon display function
...
def crash():
    ...
    pygame.mixer.music.stop() #stops music in the event of crash
...
def paused(): #Pause Screen loop
    ...
    pygame.mixer.music.pause() #pauses current music playing
...
def game_loop():
    global pause
    pygame.mixer.music.play(-1) #int defines how many times music plays, -1 is forever, 0 is once, etc
